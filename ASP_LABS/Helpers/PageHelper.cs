...
namespace ASP_LABS.Helpers
{
	...
	public class PageHelper :TagHelper
	{
		...
		public override void Process(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (!Admin.Value)
			{
				leftUrl = _linkGenerator.GetUriByAction(_httpContext, action: "Index", controller: "Book", values: new { genre = Genre, page = CurrentPage - 1 });
				rightUrl = _linkGenerator.GetUriByAction(_httpContext, action: "Index", controller: "Book", values: new { genre = Genre, page = CurrentPage + 1 });
			}
			else
			{
